  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
        <div className="glass-panel w-full max-w-3xl p-6 rounded-2xl border border-[#dfbd84]/40 bg-[#20302f] shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto text-xs text-[#a4b8b5]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#dfbd84]/20">
            <div className="flex items-center space-x-3">
              <div className="p-3 bg-[#182423] text-[#dfbd84] rounded-2xl border border-[#dfbd84]/30 shadow">
                <Wallet className="w-6 h-6 text-[#dfbd84]" />
              </div>
              <div>
                <div className="flex items-center space-x-2">
                  <h2 className="text-lg font-black text-[#f4eee2]">
                ...
                <p className="text-[11px] font-mono text-[#dfbd84] mt-0.5">
                  Master Ledger Account ID: <strong className="text-[#dfbd84]">{customer.account_number || `KSBC-ACC-${customer.id.slice(0, 8)}`}</strong>
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-2 self-end sm:self-auto">
              ...
              <button
                onClick={onClose}
                className="p-1.5 bg-[#182423] text-[#a4b8b5] hover:text-white rounded-xl border border-[#dfbd84]/30 transition"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
          ...
          <div className={`p-4 rounded-2xl border transition-all ${
            isIdUnlocked
              ...
              : 'bg-[#182423]/90 border-amber-500/40 shadow-lg'
          }`}>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
              <div className="space-y-1">
                <div className="flex items-center space-x-2">
                  <Shield className={`w-4 h-4 ${isIdUnlocked ? 'text-[#58b388]' : 'text-amber-400'}`} />
                  <span className="text-[11px] uppercase font-bold text-[#f4eee2] tracking-wider">
                    Customer User Identification Number (SSN / EIN / Tax ID)
                  </span>
                </div>
                <div className="flex items-center space-x-3 pt-1">
                  <span className="font-mono text-xl font-black text-[#dfbd84] tracking-widest">
              ...
              <div>
                {!isIdUnlocked ? (
                  <button
                    ...
                    className="px-4 py-2.5 bg-gradient-to-r from-amber-600 via-amber-500 to-amber-600 hover:from-amber-500 hover:to-amber-400 text-[#1b2827] font-black rounded-xl shadow-lg border border-amber-300/50 transition flex items-center space-x-2 text-xs"
                  >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="glass-panel p-4 rounded-2xl border border-[#dfbd84]/30 bg-[#182423]/60 space-y-3">
              <h3 className="text-xs font-bold text-[#dfbd84] uppercase tracking-wider flex items-center space-x-2 border-b border-[#dfbd84]/20 pb-2">
                <User className="w-4 h-4 text-[#dfbd84]" />
                <span>Customer Profile & Contact Information</span>
              </h3>

              <div className="space-y-2 text-xs">
                <div className="flex justify-between py-1 border-b border-[#dfbd84]/10">
                  <span className="text-[#a4b8b5]">First Name / Entity:</span>
                  <span className="font-bold text-[#f4eee2]">{customer.first_name}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-[#dfbd84]/10">
                  <span className="text-[#a4b8b5]">Last Name / Suffix:</span>
                  <span className="font-bold text-[#f4eee2]">{customer.last_name}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-[#dfbd84]/10">
                  <span className="text-[#a4b8b5]">Email Address:</span>
                  <span className="font-mono text-[#dfbd84] font-bold">{customer.email}</span>
                </div>
                <div className="flex justify-between py-1 border-b border-[#dfbd84]/10">
                  <span className="text-[#a4b8b5]">Telephone Contact:</span>
                  <span className="font-mono text-[#f4eee2]">{customer.phone || '+1-555-0199'}</span>
                </div>
                <div className="flex justify-between py-1">
                  <span className="text-[#a4b8b5]">Client Tier Category:</span>
          ...
          <div className="glass-panel p-4 rounded-2xl border border-[#dfbd84]/30 bg-[#182423]/60 space-y-3">
            <h3 className="text-xs font-bold text-[#dfbd84] uppercase tracking-wider flex items-center space-x-2 border-b border-[#dfbd84]/20 pb-2">
              <ShieldCheck className="w-4 h-4 text-[#dfbd84]" />
              <span>KYC Compliance Clearance & Beneficial Ownership Audit Notes</span>
            </h3>
            <p className="text-xs text-[#f4eee2] leading-relaxed font-mono bg-[#20302f] p-3 rounded-xl border border-[#dfbd84]/20">
          ...
          {/* Action Footer */}
          <div className="pt-3 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-[#dfbd84]/20">
            <div className="text-[11px] text-[#dfbd84]">
              Authorized Persona: <strong className="text-[#dfbd84]">{user?.first_name} {user?.last_name}</strong> ({user?.role})
            </div>
            ...
            <div className="flex items-center space-x-2">
              {isCfoOrAdmin && (
                <>
                  <button
                    ...
                    className="px-3.5 py-2 bg-[#182423] hover:bg-[#20302f] text-[#dfbd84] border border-[#dfbd84]/40 rounded-xl font-bold transition flex items-center space-x-1.5 shadow"
                  >
              ...
              <button
                onClick={onClose}
                className="px-4 py-2 bg-gradient-to-r from-[#c59e5f] to-[#dfbd84] text-[#1b2827] font-extrabold rounded-xl shadow transition"
              >
                Close Detailed View
              </button>
            </div>
