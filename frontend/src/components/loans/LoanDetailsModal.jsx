...
=====================================================`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
      <div className="glass-panel w-full max-w-4xl p-6 rounded-3xl border border-[#dfbd84]/40 bg-[#20302f] shadow-2xl space-y-5 max-h-[92vh] overflow-y-auto text-xs text-[#a4b8b5]">
        
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#dfbd84]/20">
          <div className="flex items-center space-x-3">
            <div className="p-3 bg-[#182423] text-[#dfbd84] rounded-2xl border border-[#dfbd84]/30 shadow-md">
              <Landmark className="w-6 h-6 text-[#dfbd84]" />
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <h2 className="text-xl font-black text-[#f4eee2]">
                  {applicantName}
                </h2>
                <span className="font-mono text-xs px-2 py-0.5 rounded bg-[#182423] text-[#dfbd84] font-bold border border-[#dfbd84]/30">
                  Ref #{loanIdStr}
                </span>
              </div>
              <p className="text-[11px] text-[#dfbd84] font-mono mt-0.5">
                Full UUID: <span className="text-[#f4eee2] select-all">{fullId}</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 self-end sm:self-auto">
            <div className="text-right space-y-1">
              <span className={`px-3 py-1 rounded-full text-[10px] font-extrabold uppercase border inline-flex items-center space-x-1 ${
                status === 'approved'
                  ...
                  : 'bg-[#dfbd84]/20 text-[#dfbd84] border-[#dfbd84]/40'
              }`}>
            ...
            <button
              onClick={onClose}
              className="p-1.5 bg-[#182423] text-[#a4b8b5] hover:text-white rounded-xl border border-[#dfbd84]/30 transition cursor-pointer"
              title="Close Modal"
            >
              <X className="w-5 h-5" />
            </button>
        ...
        {/* Feedback Message */}
        {feedbackMsg && (
          <div className="p-3 bg-[#182423] border border-[#dfbd84]/40 rounded-xl text-xs font-bold text-[#dfbd84] flex items-center justify-between">
            <span>{feedbackMsg}</span>
            <button onClick={() => setFeedbackMsg('')} className="text-[#a4b8b5] hover:text-white">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="p-3.5 bg-[#182423]/90 rounded-2xl border border-[#dfbd84]/30">
            <span className="text-[10px] text-[#a4b8b5] uppercase font-bold block">PRINCIPAL REQUESTED</span>
            <p className="text-xl font-black text-[#dfbd84] font-mono mt-1">
              ${principal.toLocaleString()}
            </p>
            <span className="text-[10px] text-[#a4b8b5]">Commercial Facility</span>
          </div>

          <div className="p-3.5 bg-[#182423]/90 rounded-2xl border border-[#dfbd84]/30">
            <span className="text-[10px] text-[#a4b8b5] uppercase font-bold block">RATE & TERM</span>
            <p className="text-xl font-black text-[#f4eee2] font-mono mt-1">
              {rate}% <span className="text-xs font-normal">({term}m)</span>
            </p>
            <span className="text-[10px] text-[#a4b8b5]">{(term / 12).toFixed(1)} Year Amortization</span>
          </div>

          <div className="p-3.5 bg-[#182423]/90 rounded-2xl border border-[#dfbd84]/30">
            <span className="text-[10px] text-[#a4b8b5] uppercase font-bold block">MONTHLY PAYMENT (EMI)</span>
            <p className="text-xl font-black text-[#58b388] font-mono mt-1">
              ${monthlyPayment.toLocaleString()}
            </p>
            <span className="text-[10px] text-[#a4b8b5]">Est. Debt Service</span>
          </div>

          <div className="p-3.5 bg-[#182423]/90 rounded-2xl border border-[#dfbd84]/30">
            <span className="text-[10px] text-[#a4b8b5] uppercase font-bold block">TOTAL REPAYMENT</span>
            <p className="text-xl font-black text-[#f4eee2] font-mono mt-1">
              ${totalRepayment.toLocaleString()}
            </p>
            <span className="text-[10px] text-[#dfbd84]">Interest: ${totalInterest.toLocaleString()}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div className="p-4 bg-[#182423]/70 rounded-2xl border border-[#dfbd84]/25 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-[#dfbd84]/20">
              <h3 className="font-extrabold text-[#f4eee2] text-xs uppercase tracking-wider flex items-center space-x-1.5">
                <Building className="w-4 h-4 text-[#dfbd84]" />
                <span>Borrower Profile & Facility Credentials</span>
              </h3>
              <span className="px-2 py-0.5 rounded text-[9px] font-bold uppercase bg-[#20302f] text-[#dfbd84] border border-[#dfbd84]/30">
                {category.replace('_', ' ')}
              </span>
            </div>
            ...
            <div className="space-y-2.5 text-xs">
              <div className="flex justify-between items-center">
                <span className="text-[#a4b8b5]">Account Holder:</span>
                <span className="font-bold text-[#f4eee2]">{applicantName}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-[#a4b8b5]">Master Account #:</span>
                <span className="font-mono font-bold text-[#dfbd84]">{accountNumber}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-[#a4b8b5]">National Tax / EIN ID:</span>
                <div className="flex items-center space-x-1.5">
                  <span className="font-mono text-[#f4eee2]">{maskedTaxId}</span>
                  <button
                    type="button"
                    onClick={() => setIsTaxIdVisible(!isTaxIdVisible)}
                    className="text-[#dfbd84] hover:text-white p-0.5 cursor-pointer"
              ...
              <div className="flex justify-between items-center">
                <span className="text-[#a4b8b5]">Annual Revenue / Income:</span>
                <span className="font-mono font-bold text-[#58b388]">${annualRevenue.toLocaleString()}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-[#a4b8b5]">Loan Purpose:</span>
                <span className="font-semibold text-[#f4eee2] text-right max-w-[200px] truncate">{purpose}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-[#a4b8b5]">Collateral Valuation:</span>
                <span className="font-mono text-[#dfbd84]">${collateralValue.toLocaleString()} (125% Buffer)</span>
              </div>

              <div className="pt-2 border-t border-[#dfbd84]/15">
                <span className="text-[10px] text-[#a4b8b5] block mb-1 font-bold uppercase">Decision Notes / Status Rationale:</span>
                <p className="text-[11px] italic text-[#dfbd84] bg-[#182423] p-2 rounded-xl border border-[#dfbd84]/20">
          ...
          <div className="p-4 bg-[#182423]/70 rounded-2xl border border-[#dfbd84]/25 space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-[#dfbd84]/20">
              ...
              <button
                ...
                className="px-2 py-0.5 bg-[#dfbd84]/20 hover:bg-[#dfbd84]/30 text-[#dfbd84] rounded text-[10px] font-bold border border-[#dfbd84]/40 flex items-center space-x-1 transition cursor-pointer"
              >
            ...
            <div className="p-3 bg-[#182423] rounded-xl border border-[#dfbd84]/30 flex items-center justify-between">
              <div>
                <span className="text-[9px] text-[#a4b8b5] font-bold uppercase block">AI RISK SCORE</span>
                <div className="flex items-baseline space-x-1.5 mt-0.5">
                  <span className={`text-2xl font-black font-mono ${
                    score > 70 ? 'text-red-400' : score > 40 ? 'text-[#dfbd84]' : 'text-emerald-400'
                  }`}>
                    {score}
                  </span>
                  <span className="text-xs text-[#a4b8b5] font-mono">/ 100</span>
                </div>
              </div>

              <div className="text-right">
                <span className="text-[9px] text-[#a4b8b5] font-bold uppercase block">UNDERWRITING ADVISORY</span>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="p-2 bg-[#20302f]/80 rounded-lg border border-[#dfbd84]/20">
                <span className="text-[9px] text-[#a4b8b5] block">DEFAULT PROBABILITY</span>
                <span className="font-mono font-bold text-[#f4eee2]">{defaultProb}%</span>
              </div>
              <div className="p-2 bg-[#20302f]/80 rounded-lg border border-[#dfbd84]/20">
                <span className="text-[9px] text-[#a4b8b5] block">DTI BURDEN RATIO</span>
                <span className="font-mono font-bold text-[#dfbd84]">{(dtiRatio * 100).toFixed(1)}%</span>
              </div>
              <div className="p-2 bg-[#20302f]/80 rounded-lg border border-[#dfbd84]/20">
                <span className="text-[9px] text-[#a4b8b5] block">RECOMMENDED BORROWING CAP</span>
                <span className="font-mono font-bold text-[#58b388]">${maxRecommended.toLocaleString()}</span>
              </div>
              <div className="p-2 bg-[#20302f]/80 rounded-lg border border-[#dfbd84]/20">
                <span className="text-[9px] text-[#a4b8b5] block">LOAN-TO-VALUE (LTV)</span>
                <span className="font-mono font-bold text-[#f4eee2]">80.0%</span>
              </div>
            </div>

            {/* AI Summary Quote */}
            {aiData.summaryAdvisory && (
              <p className="text-[10px] italic text-[#a4b8b5] bg-[#182423] p-2 rounded-lg border border-[#dfbd84]/15">
                🤖 "{aiData.summaryAdvisory}"
              </p>
            )}
          </div>
        ...
        {activeActionPrompt && (
          <div className="p-4 bg-[#182423] rounded-2xl border border-[#dfbd84]/50 space-y-3 animate-fadeIn shadow-xl">
            <div className="flex items-center justify-between">
              <h4 className="font-extrabold text-[#f4eee2] text-xs uppercase flex items-center space-x-2">
                <span>Confirm Action: {activeActionPrompt.toUpperCase().replace('_', ' ')}</span>
              </h4>
              <button onClick={() => setActiveActionPrompt(null)} className="text-[#a4b8b5] hover:text-white">
                <X className="w-4 h-4" />
              </button>
            </div>

            <div className="space-y-1.5">
              <label className="block text-[10px] text-[#dfbd84] font-bold uppercase">
              ...
              <input
                ...
                className="w-full glass-input bg-[#1b2827] text-white rounded-xl p-2.5 text-xs"
              />
            </div>
        ...
        <div className="pt-4 border-t border-[#dfbd84]/20 flex flex-wrap items-center justify-between gap-3">
          {/* Left Utilities */}
          <div className="flex items-center space-x-2">
            <button
              type="button"
              onClick={handleExportSlip}
              className="px-3 py-2 bg-[#182423] hover:bg-[#20302f] text-[#dfbd84] rounded-xl border border-[#dfbd84]/30 font-bold transition flex items-center space-x-1.5 cursor-pointer text-xs"
            >
            ...
            {status === 'approved' && onDisburse && (
              <button
                ...
                disabled={isProcessing}
                className="px-3.5 py-2 bg-gradient-to-r from-[#c59e5f] to-[#58b388] hover:from-[#dfbd84] hover:to-emerald-400 text-[#1b2827] rounded-xl font-black transition flex items-center space-x-1.5 cursor-pointer text-xs disabled:opacity-50"
              >
                <DollarSign className="w-3.5 h-3.5 text-[#1b2827]" />
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="px-3.5 py-2 bg-[#58b388] hover:bg-emerald-400 text-[#1b2827] text-xs font-black rounded-xl transition flex items-center space-x-1.5 disabled:opacity-30 cursor-pointer shadow-md"
            >
            ...
            <button
              ...
              className="px-3.5 py-2 bg-amber-500 hover:bg-amber-400 text-[#1b2827] text-xs font-black rounded-xl transition flex items-center space-x-1.5 disabled:opacity-30 cursor-pointer shadow-md"
            >
