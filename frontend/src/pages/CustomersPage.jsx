    ...
    <div className="flex min-h-screen bg-[#1b2827] text-[#a4b8b5]">
        ...
        <main className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-[#dfbd84]/70">KSBC Corporate Client Records ({customers.length} Accounts Onboarded)</p>
            </div>

            <div className="flex items-center space-x-3">
              <button
                onClick={() => setIsAuthModalOpen(true)}
                className="px-3.5 py-2 bg-rose-950 hover:bg-[#324846] text-amber-300 text-xs font-bold rounded-xl border border-rose-800/60 transition flex items-center space-x-1.5"
              >
              ...
              <button
                ...
                className="px-4 py-2.5 bg-gradient-to-r from-[#c59e5f] via-[#dfbd84] to-[#c59e5f] hover:from-[#dfbd84] hover:to-[#eed29e] text-white text-xs font-bold rounded-xl shadow-lg shadow-[#182423]/30 transition flex items-center space-x-2 border border-[#dfbd84]/50"
              >
          ...
          {loading ? (
            ...
            <div className="glass-panel rounded-2xl border border-[#dfbd84]/25 overflow-hidden space-y-4 p-5">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <h3 className="text-sm font-bold text-white flex items-center space-x-2">
                  <Users className="w-4 h-4 text-[#dfbd84]" />
                  <span>Onboarded Corporate Accounts ({filteredCustomers.length} displayed of {customers.length})</span>
                </h3>

                {/* Search Bar */}
                <div className="relative w-full md:w-72">
                  <Search className="absolute left-3 top-2.5 w-4 h-4 text-[#dfbd84]/60" />
                  <input
                    type="text"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    placeholder="Search client, EIN, or email..."
                    className="w-full glass-input bg-[#1a030b] border border-[#dfbd84]/30 text-xs rounded-xl p-2 pl-9 text-white focus:outline-none focus:ring-2 focus:ring-rose-600"
                  />
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="border-b border-[#dfbd84]/25 text-[#dfbd84]/70 uppercase tracking-wider text-[10px] bg-[#182423]">
                    ...
                    {filteredCustomers.map((c) => (
                      <tr key={c.id} className="hover:bg-rose-950/30 transition">
                        <td className="py-3.5 px-4">
                          ...
                          <span className="text-[10px] text-[#dfbd84]/60 font-mono">ID: {c.id.slice(0, 8)}</span>
                        </td>
                        ...
                        <td className="py-3.5 px-4 space-y-0.5">
                          <div className="flex items-center space-x-1.5 text-slate-300">
                            <Mail className="w-3 h-3 text-[#dfbd84]/60" />
                            <span>{c.email}</span>
                          </div>
                          <div className="flex items-center space-x-1.5 text-slate-400 text-[11px]">
                            <Phone className="w-3 h-3 text-[#dfbd84]/60" />
                            <span>{c.phone}</span>
                          </div>
                        </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md p-6 rounded-2xl border border-[#dfbd84]/30 relative">
            <div className="flex items-center justify-between pb-3 border-b border-[#dfbd84]/25 mb-4">
            ...
            <form onSubmit={handleOnboardCustomer} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full glass-input bg-[#1a030b] border border-[#dfbd84]/30 rounded-xl p-2.5 text-white"
                  required
              ...
              <div className="pt-3 flex justify-end space-x-2 border-t border-[#dfbd84]/25">
                <button
                  type="button"
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 bg-[#273a39] hover:bg-rose-700 text-white rounded-xl font-bold transition disabled:opacity-50"
                >
