    ...
    <div className="flex min-h-screen bg-[#120207] text-slate-100">
        ...
        <main className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-rose-300/60">KSBC Corporate Client Records ({customers.length} Accounts Onboarded)</p>
            </div>

            <div className="flex items-center space-x-3">
              <button
                onClick={() => setIsAuthModalOpen(true)}
                className="px-3.5 py-2 bg-rose-950 hover:bg-rose-900/60 text-amber-300 text-xs font-bold rounded-xl border border-rose-800/60 transition flex items-center space-x-1.5"
              >
          ...
          {loading ? (
            ...
            <div className="glass-panel rounded-2xl border border-rose-900/30 overflow-hidden space-y-4 p-5">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    {filteredCustomers.map((c) => (
                      <tr key={c.id} className="hover:bg-rose-950/30 transition">
                        <td className="py-3.5 px-4">
                          ...
                          <span className="text-[10px] text-rose-400/60 font-mono">ID: {c.id.slice(0, 8)}</span>
                        </td>
                        ...
                        <td className="py-3.5 px-4 space-y-0.5">
                          ...
                          <div className="flex items-center space-x-1.5 text-slate-400 text-[11px]">
                            <Phone className="w-3 h-3 text-rose-400/60" />
                            <span>{c.phone}</span>
                          </div>
                        </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md p-6 rounded-2xl border border-rose-900/40 relative">
            <div className="flex items-center justify-between pb-3 border-b border-rose-900/30 mb-4">
            ...
            <form onSubmit={handleOnboardCustomer} className="space-y-3 text-xs">
              ...
              <div className="pt-3 flex justify-end space-x-2 border-t border-rose-900/30">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 bg-rose-800 hover:bg-rose-700 text-white rounded-xl font-bold transition disabled:opacity-50"
                >
