      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <main className="p-6 md:p-8 space-y-6 max-w-6xl mx-auto w-full flex-1 flex flex-col">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-black text-[#1E2748] font-heading flex items-center space-x-2">
                ...
                <span className="text-xs px-2.5 py-0.5 rounded-full bg-[#EBE4CD]/10 text-[#1E2748] border border-[#1E2748]/15 font-mono font-bold">
                  CFO Clearance Only
                </span>
              </h1>
            ...
            <div className="flex items-center space-x-2 px-3 py-1.5 bg-[#F6F2E3] text-[#1E2748] border border-[#1E2748]/15 rounded-xl text-xs text-[#1E2748] font-bold">
              <span className="w-2 h-2 rounded-full bg-[#EBE4CD] animate-pulse"></span>
          ...
          <div className="flex-1 glass-panel rounded-2xl border border-[#1E2748]/15 flex flex-col h-[550px] overflow-hidden bg-[#F6F2E3] text-[#1E2748]/80 shadow-2xl">
            <div className="p-4 border-b border-[#1E2748]/15 bg-[#F6F2E3] text-[#1E2748]/90 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="w-9 h-9 rounded-2xl bg-gradient-to-br from-[#1E2748] via-[#1E2748] to-[#1E2748] flex items-center justify-center shadow-lg p-0.5">
                  <div className="w-full h-full bg-[#EBE4CD] rounded-[14px] flex items-center justify-center">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                >
                  <div className={`max-w-xl p-4 rounded-2xl text-xs space-y-2 ${
                    ...
                    {m.suggestedQueries && m.suggestedQueries.length > 0 && (
                      <div className="pt-3 flex flex-wrap gap-1.5 border-t border-[#1E2748]/15 mt-2">
              ...
              {loading && (
                <div className="flex justify-start">
                  <div className="bg-[#EBE4CD] p-4 rounded-2xl rounded-tl-none border border-[#1E2748]/15">
            ...
            {/* Input Form */}
            <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="p-3 border-t border-[#1E2748]/15 bg-[#F6F2E3] text-[#1E2748]/90 flex items-center space-x-2">
              <input
                type="text"
                value={inputMessage}
                onChange={(e) => setInputMessage(e.target.value)}
                placeholder="Ask about Vault Reserves, Loan Portfolio Yield, CAR, or NPA provisions..."
                className="flex-1 glass-input bg-[#EBE4CD] border border-[#1E2748]/15 text-xs rounded-xl p-3 text-[#1E2748] focus:outline-none focus:ring-2 focus:ring-[#1E2748]"
              />
              <button
                type="submit"
                disabled={loading || !inputMessage.trim()}
                className="px-5 py-3 bg-[#1E2748] hover:bg-[#141C33] text-[#FAF7E6] font-archivo font-extrabold font-black rounded-xl shadow-lg transition flex items-center justify-center space-x-1 disabled:opacity-50 border border-[#1E2748]/15"
              >
                <Send className="w-4 h-4 text-[#1E2748]" />
              </button>
            </form>
          </div>
        </main>
