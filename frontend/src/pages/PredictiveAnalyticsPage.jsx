      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <main className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            {/* Scenario Selector */}
            <div className="flex items-center space-x-2 bg-[#073642] p-1.5 rounded-xl border border-[#2aa198]/30 text-xs">
              <span className="text-[10px] font-bold text-[#2aa198] uppercase px-2">Stress Scenario:</span>
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg font-bold transition text-xs ${
                    scenario === s.id
                      ? 'bg-[#002129] text-[#ffd700] border border-[#ffd700]/40 shadow'
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              ...
              <div className="glass-panel p-5 rounded-2xl border border-[#ffd700]/40 bg-[#073642]/80 space-y-2 shadow-xl">
                <div className="flex items-center space-x-2 text-[#ffd700]">
                  <Sparkles className="w-5 h-5 text-[#ffd700]" />
                  <h3 className="text-sm font-bold text-[#fdf6e3]">Gemini AI 12-Month Predictive Executive Summary</h3>
                </div>
                <div className="text-xs text-[#fdf6e3] whitespace-pre-line leading-relaxed font-sans bg-[#002129] p-4 rounded-xl border border-[#2aa198]/30">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="p-4 bg-[#073642]/60 rounded-2xl border border-[#2aa198]/30 space-y-1">
                  <span className="text-[10px] font-bold text-[#2aa198] uppercase block">PROJECTED DEPOSITS (12M)</span>
                  <span className="text-xl font-black text-[#ffd700] font-mono">
                    ${(analytics.projectedDepositCurve[11] / 1000000).toFixed(2)}M
                  </span>
                  <span className="text-[10px] text-[#859900] block font-bold">+18.0% Cumulative Growth</span>
                </div>

                <div className="p-4 bg-[#073642]/60 rounded-2xl border border-[#2aa198]/30 space-y-1">
                  <span className="text-[10px] font-bold text-[#2aa198] uppercase block">PROJECTED PORTFOLIO (12M)</span>
                  <span className="text-xl font-black text-[#fdf6e3] font-mono">
                    ${(analytics.projectedPortfolioCurve[11] / 1000000).toFixed(2)}M
                  </span>
                  <span className="text-[10px] text-[#859900] block font-bold">+26.4% Expansion</span>
                </div>

                <div className="p-4 bg-[#073642]/60 rounded-2xl border border-[#2aa198]/30 space-y-1">
                  <span className="text-[10px] font-bold text-[#2aa198] uppercase block">PROBABILITY OF DEFAULT (PD)</span>
                  <span className="text-xl font-black text-amber-300 font-mono">
                    {analytics.stressTestMetrics.probabilityOfDefault}
                  </span>
                  <span className="text-[10px] text-[#93a1a1] block font-medium">ECL Provision: ${analytics.stressTestMetrics.expectedCreditLossAmount.toLocaleString()}</span>
                </div>

                <div className="p-4 bg-[#073642]/60 rounded-2xl border border-[#2aa198]/30 space-y-1">
                  <span className="text-[10px] font-bold text-[#2aa198] uppercase block">TIER-1 CAPITAL ADEQUACY</span>
                  <span className="text-xl font-black text-[#859900] font-mono">
                    {analytics.stressTestMetrics.tier1CapitalCoverage}
                  </span>
                  <span className="text-[10px] text-[#859900] block font-bold">LCR: {analytics.stressTestMetrics.liquidityCoverageRatio}</span>
                </div>
              </div>
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-[#2aa198]/15">
                      {analytics.months.map((m, idx) => (
                        <tr key={idx} className="hover:bg-[#002129]/40 transition font-mono">
                          <td className="py-3 px-3 font-bold text-[#ffd700] font-sans">{m}</td>
                          <td className="py-3 px-3 text-right font-extrabold text-[#fdf6e3]">
                            ${analytics.projectedDepositCurve[idx].toLocaleString()}
                          </td>
                          <td className="py-3 px-3 text-right font-extrabold text-[#859900]">
                            ${analytics.projectedPortfolioCurve[idx].toLocaleString()}
                          </td>
                          <td className="py-3 px-3 text-center text-[#ffd700] font-bold">
                            {analytics.netInterestMarginCurve[idx]}% APR
                          </td>
                          <td className="py-3 px-3 text-center">
                            <span className="px-2 py-0.5 rounded text-[9px] font-bold border uppercase bg-[#859900]/20 text-[#859900] border-[#859900]/40 font-sans">
                              Forecast
                            </span>
                          </td>
                        </tr>
