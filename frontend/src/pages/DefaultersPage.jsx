        ...
        <main className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-rose-300/60">Early Warning Indicator & AI Non-Performing Asset (NPA) Restructuring Workout Plans</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="lg:col-span-7 glass-panel p-5 rounded-2xl border border-rose-900/30 space-y-4">
                ...
                <div className="space-y-3">
                  {defaulters.map((d) => (
                    <div key={d.id} className="p-4 bg-rose-950/40 rounded-xl border border-rose-900/40 space-y-3 hover:border-red-600/50 transition">
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="font-bold text-white text-sm">{d.borrowerName}</h4>
                          <span className="text-[10px] font-mono text-rose-300/70">ID: {d.id} | Collateral: {d.collateralType}</span>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs font-mono p-2.5 bg-rose-950/60 rounded-lg">
                        <div>
                ...
                {activeStrategy ? (
                  <div className="glass-panel p-6 rounded-2xl border border-rose-900/40 space-y-4 shadow-2xl bg-rose-950/20">
                    ...
                    <div className="space-y-2 text-xs">
                      <span className="font-bold text-rose-300 block text-[11px] uppercase tracking-wider">Recommended Loss Mitigation Actions:</span>
                      {activeStrategy.recommendedActions?.map((act, i) => (
                        <div key={i} className="p-2 bg-rose-950/50 rounded-lg border border-rose-900/40 text-slate-200 flex items-center space-x-2">
